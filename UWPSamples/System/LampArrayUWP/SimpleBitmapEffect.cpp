		...
		Windows::Graphics::Imaging::BitmapBuffer^ buffer = bitmap->LockBuffer(Windows::Graphics::Imaging::BitmapBufferAccessMode::Write);
		{
			Windows::Foundation::IMemoryBufferReference^ reference = buffer->CreateReference();
			{
				...
				if (result == S_OK)
				{
					byte* dataInBytes;
					uint32_t capacity;
					bufferByteAccess->GetBuffer(&dataInBytes, &capacity);
					for (uint32_t i = 0; i < bitmapByteCount; i += 4)
					{
						...
						if (renderBlue)
						{
							...
							dataInBytes[i] = 0x00;      // R
							dataInBytes[i + 1] = 0x00;  // G
							dataInBytes[i + 2] = 0xFF;  // B
							dataInBytes[i + 3] = 0xFF;  // A
						}
						else
						{
							// Yellow.  All colors must be in RGBA8 format.
							dataInBytes[i] = 0xFF;      // R
							dataInBytes[i + 1] = 0xFF;  // G
							dataInBytes[i + 2] = 0x00;  // B
							dataInBytes[i + 3] = 0xFF;  // A
						}
					}
				}
